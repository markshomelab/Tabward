// background.js — service worker for New Tab Focus extension
...
chrome.runtime.onInstalled.addListener(() => {
  chrome.storage.local.get(ENABLED_KEY, (result) => {
    ...
    updateIcon(result[ENABLED_KEY] !== false);
  });
});
...
chrome.tabs.onCreated.addListener((tab) => {
  chrome.storage.local.get(ENABLED_KEY, (result) => {
    ...
    if (isEnabled && tab.id !== undefined) {
      // Use a small delay to let Chrome finish creating the tab before focusing
      setTimeout(() => {
...
chrome.action.onClicked.addListener(() => {
  chrome.storage.local.get(ENABLED_KEY, (result) => {
    const newState = !(result[ENABLED_KEY] !== false);
